...
while key != 27:  # While Esc key is not pressed
    ...
    win.timeout(150 - (len(snake)/5 + len(snake)/10) % 120)  # Increases the
    # speed of Snake as its length increases
    ...
    if key not in [
                    KEY_LEFT, KEY_RIGHT,
                    KEY_UP, KEY_DOWN, 27]:  # If an invalid key is pressed
        key = prevKey
    ...
    if snake[0] == food:  # When snake eats the food
        ...
        while food == []:
            food = [randint(1, 18),
                    randint(1, 58)]  # Calculating next food's coordinates
            if food in snake:
                food = []
